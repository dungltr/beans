...
import gr.ntua.cslab.asap.utils.ParetoPlan;
			...
			if( l != null && !l.isEmpty()){
				if(functionTarget.contains("min")){
					...
					for(WorkflowNode r : l){
						...
						//goodPlan.add(tempPlan);
                                                goodPlan = ParetoPlan.buildGoodPlan (dpTable, r, fullName, goodPlan, index);
				...
				*/
				...
				//System.out.println("Optimal cost: "+bestCost);
                                goodPlan = ParetoPlan.ParetoSet(Library+"/"+MaterializedWorkflowLibrary.getWorkflowDirectory()+"/"+fullName, goodPlan);
				for (int i = 0; i < goodPlan.size(); i++){
                                    materializedWorkflow.addParetorPlan(t.toStringNorecursive(), goodPlan.get(i));
                                }
                ...
                /*Dung edit begin*/
//                ParetoPlan.ParetoSet(Library+"/"+MaterializedWorkflowLibrary.getWorkflowDirectory()+"/"+fullName, goodPlan);
//                for (int i = 0; i< ParetoSet.size(); i++){
//                    materializedWorkflow.addParetorPlan(t.toStringNorecursive(), ParetoSet.get(i));
//                }
